...
<head>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#doesNotExpire").change(function(){
                if ($(this).prop("checked")== true){
                    $("#expDate").val("2025-01-01");
                    $("#expDate").prop("readonly", true);
                    $("#expDate").css("background-color", "gray");
                }
                else {
                    var today = new Date('yy-mm-dd');
                    $("#expDate").val(today);
                    $("#expDate").prop("readonly", false);
                    $("#expDate").css("background-color", "inherit");
                }
            })
        })
    </script>
        ...
        <div id="content">
            <div id="innerContent">
                <form method="POST" action="submit.php">
                    ...
                    <label>Does Not Expire: </label><input type="checkbox" name="doesNotExpire" id="doesNotExpire" />
                    <input type="hidden" name="cust_id" value="<?php echo $doc_info['cust_id']?>" />
                    <span id="warning"></span><br style="clear:both" />
